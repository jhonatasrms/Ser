...
export const TopBar: React.FC<LayoutProps> = ({ currentView, onNavigate, userPoints, hasUser }) => {
  ...
  return (
    <nav className="sticky top-0 z-40 w-full bg-brand-bg/95 backdrop-blur-md border-b border-brand-primary/10 shadow-sm">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <div className="flex items-center cursor-pointer" onClick={() => onNavigate('home')}>
            <Heart className="text-brand-primary mr-2 fill-current" size={24} />
            <div>
          ...
          {userPoints !== undefined && hasUser && (
            <div className="flex items-center px-4 py-1.5 bg-brand-card text-brand-primary rounded-full font-bold border border-brand-primary/20 shadow-sm">
  ...
  return (
    <div className="md:hidden fixed bottom-0 w-full bg-brand-card border-t border-brand-primary/10 z-50 flex justify-around py-3 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
       <button 
         ...
         className={`flex flex-col items-center space-y-1 ${currentView === 'home' ? 'text-brand-primary' : 'text-brand-textSec/60'}`}
       >
         <Home size={22} />
       ...
       {hasUser && (
         <button 
           ...
           className={`flex flex-col items-center space-y-1 ${currentView === 'dashboard' ? 'text-brand-primary' : 'text-brand-textSec/60'}`}
         >
       ...
       <button 
         ...
         className={`flex flex-col items-center space-y-1 ${currentView === 'pricing' ? 'text-brand-primary' : 'text-brand-textSec/60'}`}
       >
         <CreditCard size={22} />
